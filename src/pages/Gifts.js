import React from 'react';
import Sidenav from './Sidenav';

export default class Gifts extends React.Component {
  render() {
    return (
      <div className="row">
        <main className="container">
          <h3> Gifts </h3>
          <p> Het is cliché… en toch herhalen wij het hier nog eens: cadeaus zijn echt-echt-echt niet nodig! Je doet ons het grootste plezier met jullie aanwezigheid. </p>
          <p> Na 7 jaar samenwonen hebben wij al alles waar wij van kunnen dromen… </p>
          <p> Voor zij die het toch echt niet kunnen laten: wij reizen zeer graag en veel dus als we dan toch in een richting moeten denken: een klein spaarcentje voor onze volgende trip <i className="tiny material-icons">sentiment_very_satisfied</i></p>
        </main>
      </div>
    );
  }
}
